...
def start_jvm(add_classpath=None, jvm_flags=[], rw_version="2021.1.0"):
    os_str = 'lin64'
    if platform.system() == 'Windows':
        os_str = 'win64'
    dir_path = os.path.dirname(os.path.realpath(__file__))
    file_name = "rapidwright-"+rw_version+"-standalone-"+os_str+".jar"
    classpath = os.path.join(dir_path,file_name)
    print("file=" + classpath)
    if not os.path.isfile(classpath):
        url = "http://github.com/Xilinx/RapidWright/releases/download/v"+rw_version+"-beta/" + file_name
