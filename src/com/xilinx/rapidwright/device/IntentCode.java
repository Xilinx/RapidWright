...
public enum IntentCode {
    ...
    public static boolean isLongWire(int intentCode){
    	IntentCode ic = getValues()[intentCode];
    ...
    public static boolean isUltraScaleClocking(Tile tile, int wire){
    	return getValues()[tile.getWireIntentCode(wire).ordinal()].isUltraScaleClocking();	
    }
    ...
    private static IntentCode[] values = values();
    
    public static IntentCode[] getValues() {
        return values;
    }
    ...
    public static IntentCode[] getIntentCodesBySeries(Series s){
    	if(s == Series.Series7){
    		return Arrays.copyOfRange(IntentCode.getValues(), SERIES7_START_IDX, SERIES7_END_IDX);
    	}else if(s == Series.UltraScale){
    		return Arrays.copyOfRange(IntentCode.getValues(), ULTRASCALE_START_IDX, ULTRASCALE_END_IDX);
    	}else if(s == Series.UltraScalePlus){
    		return Arrays.copyOfRange(IntentCode.getValues(), ULTRASCALEPLUS_START_IDX, ULTRASCALEPLUS_END_IDX);
    	} 
    	return null;
    }
