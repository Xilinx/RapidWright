...
public class PseudoPIPHelper {
    ...
    private static List<BELPin> getPath(SitePin start, SitePin end){
        ...
        if(!start.isInput()) {
            BELPin usedOutput = startBELPin.getSourcePin();
            BELPin input = usedOutput.getSitePIPs().get(0).getInputPin();
            BELPin commonPin = input.getSourcePin();
            belPins.addFirst(endBELPin);
            belPins = exploreInput(belPins, endBELPin, commonPin);
            belPins.removeFirst();
            return belPins;
        }
    ...
    public static void main(String[] args) {
        // Quick example test
        Device device = Device.getDevice(Device.PYNQ_Z1);
