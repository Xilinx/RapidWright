...
public class PartialRouter extends RWRoute{
	...
	private void handleConflictNets() {
		...
		for(Net net : conflictNets) {
			if(!isTargetConflicNetToRoute(net)) {
	 ...
	 * It is customized for the RapidStream use case, where the targets are nets connecting anchor FFs of CLB tiles.
	 * @param net The net in question.
	 * @return true if the net is a target net.
	 */
	private boolean isTargetConflicNetToRoute(Net net) {
