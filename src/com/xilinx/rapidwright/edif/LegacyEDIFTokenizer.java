...
public class LegacyEDIFTokenizer implements AutoCloseable, IEDIFTokenizer {
     ...
     * @return
     */
    private EDIFToken getQuotedToken(long tokenStart, boolean isShortLived) throws IOException {
    ...
    private EDIFToken getUnquotedToken(char startChar, long tokenStart, boolean isShortLived) throws IOException {
        buffer[0] = startChar;
        int idx = 1;
        int ch;
        while ((ch = in.read()) != -1) {
            ...
            while ((ch = in.read()) != -1) {
                ...
                switch (ch) {
                    ...
                    case '\t':
                        break;
                    default:
                        return getUnquotedToken((char) ch, byteOffset-1, isShortLived);
                }
            }
            //EOF
            return null;
    ...
    public String getOptionalNextTokenString(boolean isShortLived) {
        return getOptionalNextToken(isShortLived).text;
    }

    public EDIFToken peekOptionalNextToken(){
        if(nextTokens.isEmpty()){
            EDIFToken next = getOptionalNextToken(false);
